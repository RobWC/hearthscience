...
func allCards(w http.ResponseWriter, r *http.Request) {
	...
	var cards []hs.Card
	var err = cardsConn.Find(bson.M{"collectible": true, "$or": []bson.M{bson.M{"type": "Minion"}, bson.M{"type": "Spell"}}}).Sort("name").All(&cards)
	if err != nil {
...
func allCardsJSONv1(w http.ResponseWriter, r *http.Request) {
	cardsConn := mongoSession.DB("").C("cards")
	var cards []hs.Card
	var err = cardsConn.Find(bson.M{"collectible": true}).Sort("name").All(&cards)
	if err != nil {
		log.Printf(err.Error())
		return
	}
	newJSON, _ := json.Marshal(cards)
	fmt.Fprintf(w, "{\"total\":"+strconv.Itoa(len(cards))+" ,\"cards\":"+string(newJSON)+"}")
}
