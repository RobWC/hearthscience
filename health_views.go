...
*/
func healthCards(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	health := vars["health"]
	...
	cardsConn := mongoSession.DB("").C("cards")
	var cards []hs.Card
	intHealth, _ := strconv.Atoi(health)
	var err = cardsConn.Find(bson.M{"health": intHealth}).Sort("name").All(&cards)
	if err != nil {
		log.Printf(err.Error())
		return
	}
